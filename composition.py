...
# Human-readable Spanish explanations for each failure reason.
# Composition stays deterministic — no model involved in this mapping —
# so that the failure section reads consistently across runs.
FAILURE_REASON_LABELS_ES = {
    "api_error": (
        "Error en la API",
        "La llamada al modelo falló tras reintentos. Probablemente transitorio; "
        "se puede re-evaluar manualmente."
    ),
    "parse_error": (
        "Respuesta no parseable",
        "El modelo devolvió texto que no es JSON válido. Revisar la respuesta "
        "cruda en failures_*.json."
    ),
    "schema_invalid": (
        "Campos requeridos faltantes",
        "El modelo devolvió JSON válido pero sin todos los campos obligatorios. "
        "Probable problema de prompt — revisar la respuesta cruda."
    ),
    "timeout": (
        "Tiempo agotado",
        "La evaluación excedió el límite de 5 minutos. Reintentar manualmente."
    ),
    "unknown": (
        "Error desconocido",
        "Revisar logs y failures_*.json."
    ),
}
...
def format_failures_section(failures):
    """
    Render the failed-evaluations section. Returns empty string when there
    are no failures so the section is hidden entirely (no empty heading).
    """
    if not failures:
        return ""

    lines = [
        "## ⚠️ Evaluaciones Fallidas",
        "",
        f"_Esta semana, {len(failures)} empresa(s) no se pudieron evaluar. "
        "Quedan registradas en `failures_*.json` para revisión manual._",
        "",
    ]

    for f in failures:
        company = f.get("company_name", "<unknown>")
        hq_guess = f.get("hq_country_guess", "")
        reason = f.get("failure_reason", "unknown")
        label, explanation = FAILURE_REASON_LABELS_ES.get(
            reason, FAILURE_REASON_LABELS_ES["unknown"]
        )

        hq_suffix = f" ({hq_guess})" if hq_guess else ""
        lines.append(f"- **{company}**{hq_suffix} — *{label}*. {explanation}")
...
def compose_email(evaluations_file, failures_file=None):
    try:
        with open(evaluations_file, "r") as f:
            evaluations = json.load(f)

        # Look for matching failures file. Default: same directory, same date,
        # failures_{today}.json. Caller can override.
        if failures_file is None:
            inferred = Path(evaluations_file).parent / Path(evaluations_file).name.replace(
                "evaluations_", "failures_"
            )
            failures_file = inferred if inferred.exists() else None
        ...
        if failures_file and Path(failures_file).exists():
            try:
                ...
                print(f"[!] Could not read failures file {failures_file}: {e}")
                failures = []
        ...
        # 3. Format failures section (deterministic, no model)
        failures_section = format_failures_section(failures)

        # 4. Generate Quality Notes with Gemini
...
"""
        ...
        if failures:
            print(f"    Includes {len(failures)} failed evaluation(s).")
        return output_path
